...
class TreeBehavior extends ModelBehavior {
    ...
    protected function _setChildrenLevel(Model $Model, $id) {
        ...
        $children = $Model->children(
                $id, false, array($primaryKey, $settings['parent'], $settings['level']), $settings['left'], null, 1, -1
        );
        ...
        foreach ($children as $node) {
            ...
            $Model->updateAll(
                    array($Model->escapeField($settings['level']) => $depth), array($Model->escapeField($primaryKey) => $node[$Model->alias][$primaryKey])
            );
        }
    }
