...
class PHP_CodeSniffer {
    ...
    $verbosity = 0, $tabWidth = 0, $encoding = 'iso-8859-1', $interactive = false
    ) {
...
//end setCli()
    ...
    public function process($files, $standards, array $restrictions = array(), $local = false) {
        ...
        foreach ($standards as $standard) {
            ...
            if ($installed !== null) {
                $standard = $installed;
            } else if (is_dir($standard) === true && is_file(realpath($standard . '/ruleset.xml')) === true
            ) {
    ...
    public function processRuleset($rulesetPath, $depth = 0) {
        ...
        foreach ($ruleset->rule as $rule) {
            ...
            $includedSniffs = array_merge(
                    $includedSniffs, $this->_expandRulesetReference($rule['ref'], $rulesetDir, $depth)
            );
                ...
                foreach ($rule->exclude as $exclude) {
                    ...
                    if (count($parts) === 4) {
                        ...
                        $excludedSniffs = array_merge(
                                $excludedSniffs, $this->_expandRulesetReference($exclude['name'], $rulesetDir, ($depth + 1))
                        );
                    }
                }//end foreach
            }//end if
            ...
            if (is_dir($path) === true) {
                if ($local === true) {
                    ...
                    $di = new RecursiveIteratorIterator(
                            new RecursiveDirectoryIterator($path), 0, RecursiveIteratorIterator::CATCH_GET_CHILD
                    );
                }
    ...
    public function processFile($file, $contents = null, $restrictions = array()) {
        ...
        try {
            ...
            $phpcsFile = new PHP_CodeSniffer_File(
                    $filename, $this->_tokenListeners, $this->allowedFileExtensions, $this->ruleset, $restrictions, $this
            );
...
//end processFile()
    ...
    private function _processFile($file, $contents, $restrictions) {
        ...
        $phpcsFile = new PHP_CodeSniffer_File(
                $file, $this->_tokenListeners, $this->allowedFileExtensions, $this->ruleset, $restrictions, $this
        );
...
//end resolveSimpleToken()
    ...
    $string, $classFormat = false, $public = true, $strict = true
    ) {
...
//end getInstalledStandardPaths()
    ...
    $includeGeneric = false, $standardsDir = ''
    ) {
...
//end getInstalledStandards()
    ...
    public static function isInstalledStandard($standard) {
        ...
        if ($path !== null) {
            ...
            if (is_file($standard) === true && substr(strtolower($standard), -4) === '.xml'
            ) {
                return true;
            }
        }
