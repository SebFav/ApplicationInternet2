...
//end register()
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
        ...
        if ($tokens[$stackPtr]['code'] == T_OPEN_TAG || $tokens[$stackPtr]['code'] == T_OPEN_TAG_WITH_ECHO
        ) {
            $indent = ($tokens[$stackPtr]['column'] - 1);
            if (empty($this->_closeTagIndents) === false && $indent === $this->_closeTagIndents[0]
            ) {
        ...
        for ($i = ($scopeOpener + 1); $i < $scopeCloser; $i++) {
            ...
            if ($newline === true && $tokens[$i]['code'] !== T_WHITESPACE) {
                ...
                if (in_array($tokens[$firstToken]['code'], $this->ignoreIndentationTokens) || in_array($tokens[$firstToken]['type'], $this->ignoreIndentationTokens)
                ) {
                    continue;
                }
