...
//end process()
    ...
    $patternInfo, PHP_CodeSniffer_File $phpcsFile, $stackPtr
    ) {
        $tokens = $phpcsFile->getTokens();
                ...
                if ($pattern[$i]['type'] === 'token') {
                    if ($pattern[$i]['token'] === T_WHITESPACE) {
                        ...
                        $prev = $phpcsFile->findPrevious(
                                $ignoreTokens, $stackPtr, null, true
                        );
                    ...
                    if ($this->ignoreComments === true && in_array($tokens[$stackPtr]['code'], PHP_CodeSniffer_Tokens::$commentTokens) === true
                    ) {
                        $startComment = $phpcsFile->findPrevious(
                                PHP_CodeSniffer_Tokens::$commentTokens, ($stackPtr - 1), null, true
                        );
                        ...
                        $tokenContent = $phpcsFile->getTokensAsString(
                                $startComment, ($stackPtr - $startComment + 1)
                        );
                    ...
                    if ($tokens[$stackPtr]['code'] === T_WHITESPACE) {
                        if ($tokens[$stackPtr]['content'] !== $phpcsFile->eolChar) {
                            ...
                            if (($tokens[($stackPtr - 1)]['content'] !== $phpcsFile->eolChar) && ($this->ignoreComments === true && in_array($tokens[($stackPtr - 1)]['code'], PHP_CodeSniffer_Tokens::$commentTokens) === false)
                            ) {
                                $hasError = true;
                            } else {
                                $stackPtr--;
        ...
        for ($i = $patternInfo['listen_pos']; $i < $patternLen; $i++) {
            if ($pattern[$i]['type'] === 'token') {
                    ...
                    if ($tokens[$stackPtr]['code'] === T_WHITESPACE) {
                        if (isset($pattern[($i + 1)]) === false) {
                            ...
                            $next = $phpcsFile->findNext(
                                    PHP_CodeSniffer_Tokens::$emptyTokens, $stackPtr, null, true
                            );

                            $tokenContent = $phpcsFile->getTokensAsString(
                                    $stackPtr, ($next - $stackPtr)
                            );
                ...
                if ($pattern[$i]['to'] === 'unknown') {
                    ...
                    // Find the previous opener.
                    $next = $phpcsFile->findPrevious(
                            PHP_CodeSniffer_Tokens::$blockOpeners, $stackPtr
                    );
