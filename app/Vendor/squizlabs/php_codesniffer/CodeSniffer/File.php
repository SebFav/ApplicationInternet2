...
class PHP_CodeSniffer_File {
    ...
    $file, array $listeners, array $tokenizers, array $ruleset, array $restrictions, PHP_CodeSniffer $phpcs
    ) {
        ...
        if (isset($cliValues['showSources']) === true && $cliValues['showSources'] !== true && array_key_exists('summary', $cliValues['reports']) === true && count($cliValues['reports']) === 1
        ) {
...
//end addTokenListener()
    ...
    PHP_CodeSniffer_Sniff $listener, array $tokens
    ) {
        foreach ($tokens as $token) {
            if (isset($this->_listeners[$token]) === false) {
                continue;
...
//end detectLineEndings()
    ...
    public function addError($error, $stackPtr, $code = '', $data = array(), $severity = 0) {
        ...
        if (empty($this->restrictions) === false && in_array($sniffCode, $this->restrictions) === false && in_array($sniff, $this->restrictions) === false
        ) {
            return;
        }
    ...
    private static function _convertTabs(&$tokens, $tokenizer, $eolChar) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (strpos($tokenContent, "\t") === false) {
                ...
                $tabs = preg_split(
                        "|(\t)|", $tokenContent, -1, PREG_SPLIT_DELIM_CAPTURE
                );
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if (isset($tokenizer->scopeOpeners[$tokens[$i]['code']]) === true) {
                ...
                $i = self::_recurseScopeMap(
                                $tokens, $numTokens, $tokenizer, $eolChar, $i
                );
            }
        }//end for
...
//end _createScopeMap()
    ...
    &$tokens, $numTokens, $tokenizer, $eolChar, $stackPtr, $depth = 1, &$ignore = 0
    ) {
        $opener = null;
        ...
        for ($i = ($stackPtr + 1); $i < $numTokens; $i++) {
            ...
            if ($opener !== null && (isset($tokens[$i]['scope_opener']) === false || $tokenizer->scopeOpeners[$tokens[$stackPtr]['code']]['shared'] === true) && in_array($tokenType, $tokenizer->scopeOpeners[$currType]['end']) === true
            ) {
            ...
            if (isset($tokenizer->scopeOpeners[$tokenType]) === true) {
                ...
                if (isset($tokens[$i]['scope_condition']) === true) {
                    ...
                    $i = self::_recurseScopeMap(
                                    $tokens, $numTokens, $tokenizer, $eolChar, $i, ($depth + 1), $ignore
                    );
            ...
            if (in_array($tokenType, $tokenizer->scopeOpeners[$currType]['start']) === true && $opener === null
            ) {
                ...
                if ($tokens[$i]['line'] >= ($startLine + 3) && in_array($tokens[($i - 1)]['code'], PHP_CodeSniffer_Tokens::$emptyTokens) === false
                ) {
            ...
            if (isset($tokens[$i]['scope_condition']) === true) {
                ...
                if ($tokens[$i]['scope_opener'] === $i) {
                    ...
                    if ($lastOpener !== null && $tokens[$lastOpener]['scope_closer'] === $tokens[$i]['scope_closer']) {
                        ...
                        $isShared = in_array(
                                $tokens[$opener]['code'], $tokenizer->scopeOpeners[$thisType]['with']
                        );
        ...
        for ($i = ($opener + 1); $i <= $closer; $i++) {
            ...
            switch ($this->_tokens[$i]['code']) {
                ...
                case T_COMMA:
                    ...
                    if ($defaultStart !== null) {
                        $vars[$paramCount]['default'] = $this->getTokensAsString(
        ...
        if (isset($this->_tokens[$stackPtr]['nested_parenthesis']) === true) {
            ...
            if (isset($this->_tokens[$lastBracket]['parenthesis_owner']) === true) {
                ...
                $prev = $this->findPrevious(
                        array(T_WHITESPACE), ($this->_tokens[$lastBracket]['parenthesis_opener'] - 1), null, true
                );
...
//end getTokensAsString()
    ...
    $types, $start, $end = null, $exclude = false, $value = null, $local = false
    ) {
        $types = (array) $types;
