...
class PHP_CodeSniffer_Reports_Full implements PHP_CodeSniffer_Report {
    ...
    $report, $showSources = false, $width = 80
    ) {
        ...
        foreach ($report['messages'] as $line => $lineErrors) {
            foreach ($lineErrors as $column => $colErrors) {
                foreach ($colErrors as $error) {
                    ...
                    $errorMsg = wordwrap(
                            $message, $maxErrorSpace, PHP_EOL . $paddingLine2
                    );
...
//end generateFileReport()
    ...
    $cachedData, $totalFiles, $totalErrors, $totalWarnings, $showSources = false, $width = 80, $toScreen = true
    ) {
        if ($cachedData === '') {
            return;
        }
