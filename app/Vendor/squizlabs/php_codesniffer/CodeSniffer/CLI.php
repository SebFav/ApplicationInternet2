    ...
    public function process($values = array()) {
        ...
        if ($values['generator'] !== '') {
            ...
            foreach ($values['standard'] as $standard) {
                $phpcs->generateDocs(
                        $standard, $values['sniffs'], $values['generator']
                );
            }
        ...
        $phpcs = new PHP_CodeSniffer(
                $values['verbosity'], $values['tabWidth'], $values['encoding'], $values['interactive']
        );
        ...
        $phpcs->process(
                $values['files'], $values['standard'], $values['sniffs'], $values['local']
        );
...
//end process()
    ...
    PHP_CodeSniffer $phpcs, $reports, $showSources, $reportFile, $reportWidth
    ) {
        ...
        foreach ($reports as $report => $output) {
            ...
            $errors = $phpcs->reporting->printReport(
                    $report, $showSources, $output, $reportWidth
            );
        }
